#!/usr/bin/env python3
"""
Quick ULID benchmark runner - test specific library locally.
Usage: python quick_benchmark.py <library_name>
"""
...
def run_py_ulid_benchmark():
    ...
    try:
        ...
        # parse_results = benchmark_function(
        #     lambda: ulid.ULID.from_str(test_ulid).timestamp, iterations=50000
        # )
...
def main():
    parser = argparse.ArgumentParser(description="Quick ULID library benchmark")
    parser.add_argument(
        "library",
        choices=["ulid-python", "python-ulid", "py-ulid", "ulid-py"],
        help="Library to benchmark",
    )
    parser.add_argument("--output", "-o", help="Output JSON file (optional)")

    args = parser.parse_args()

    print(f"🚀 Benchmarking {args.library}")
    ...
    if args.library == "ulid-python":
        results = run_ulid_python_benchmark()
    elif args.library == "python-ulid":
        results = run_python_ulid_benchmark()
    elif args.library == "py-ulid":
        results = run_py_ulid_benchmark()
    elif args.library == "ulid-py":
    ...
    # Print results
    print("\n" + "=" * 50)
    print(f"RESULTS FOR {args.library.upper()}")
    print("=" * 50)

    if "error" not in results:
        gen_ops = results["generation"].get("ops_per_sec", 0)
        parse_ops = results["parsing"].get("ops_per_sec", 0)
        gen_mem = results["generation"].get("memory_overhead_mb", 0)
        parse_mem = results["parsing"].get("memory_overhead_mb", 0)

        print(f"Generation: {gen_ops:,.0f} ops/sec (Memory: +{gen_mem:.1f} MB)")
        print(f"Parsing:    {parse_ops:,.0f} ops/sec (Memory: +{parse_mem:.1f} MB)")
    else:
